...
int ygrade =0;
ygrade = imagem.at<uchar>(y-1,x-1) * (-1);
ygrade = ygrade +  imagem.at<uchar>(y,x-1) * (-2);
ygrade = ygrade +  imagem.at<uchar>(y+1,x-1) * (-1);
ygrade = ygrade +  imagem.at<uchar>(y-1,x) * 0;
    ...
    img = src.clone();
        ...
        for(int y = 1; y < src.rows - 1; y++){
            for(int x = 1; x < src.cols - 1; x++){
                ...
                sum = sqrt(pow(abs(gx),2) + pow(abs(gy),2));
                if(sum >255){
                    sum = 255;
                }
